...
def photometric_fit(
    ...
    if use_decay:
        gs_scheduling_func_dict, cam_scheduling_func_dict = optimizer_cfg.get_scheduler(
            total_steps=total_steps - decay_start
        )
    else:
    ...
    s_model.get_cate_color(color_plate=color_plate[:n_group_static].to(s_model.device))
    d_model.get_cate_color(color_plate=color_plate[n_group_static:].to(d_model.device))

    for step in tqdm(range(cfg.total_steps)):
        # * control the w correction
        if True and step == cfg.skinning_corr_start_steps:
            logger.info(
        ...
        image, alpha, info = rasterization(
            means=torch.cat((s_means, d_means), dim=0),
            quats=torch.cat((s_quats, d_quats), dim=0),
            scales=torch.cat((s_scales, d_scales), dim=0),
            opacities=torch.cat((s_opacities, d_opacities), dim=0),
            colors=torch.cat((s_shs, d_shs), dim=0),
            height=height,
            width=width,
        ...
        if corr_exe_flag:
            ...
            if GS_BACKEND in ["native_add3"]:
                ...
                rendered_xyz_map = corr_render_dict["rgb"].permute(1, 2, 0)  # H,W,3
            ...
            with torch.no_grad():
                if flow_flag:
                    ...
                    track_valid = s2d.track_mask[view_ind] & s2d.track_mask[dst_ind]
        ...
        dynamic_to_static_transfer_flag = step in photo_s2d_trans_steps and True
        ...
        if True and step in dyn_node_densify_steps:
        ...
        # error grow
        if True and step in dyn_error_grow_steps:
        ...
        if True and step in dyn_scf_prune_steps:
            d_model.prune_nodes(
                optimizer_dynamic,
        ...
        d_n_count_list.append(d_model.N if True else 0)
        d_m_count_list.append(d_model.M if True else 0)
        ...
        if viz_flag:
            if True:
                ...
                viz_path = osp.join(self.viz_dir, f"{phase_name}_step={step}_3dviz.mp4")
                viz3d_total_video(
                    cams,
                    d_model,
                    0,
        ...
        if viz_cheap_interval > 0 and (
            ...
            with torch.no_grad():
                ...
                imageio.imsave(
                    osp.join(self.viz_dir, f"{phase_name}_photo_grad_step={step}.jpg"),
                    viz_grad,
                )
                ...
                imageio.imsave(
                    osp.join(self.viz_dir, f"{phase_name}_corr_grad_step={step}.jpg"),
                    viz_grad,
                )
            ...
            for plt_i, plt_pack in enumerate(
                ...
                plt.plot(plt_pack[1]), plt.title(plt_pack[0] + f" End={value_end:.4f}")
            plt.savefig(
    ...
    s_model.save(s_save_fn)
    logger.info(f"Static model saved to {s_save_fn}")
