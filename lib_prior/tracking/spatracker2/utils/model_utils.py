...
def nearest_sample2d(im, x, y, return_inbounds=False):
    ...
    x0_clip = torch.clamp(x0, 0, max_x) # type: ignore
    x1_clip = torch.clamp(x1, 0, max_x) # type: ignore
    y0_clip = torch.clamp(y0, 0, max_y) # type: ignore
    y1_clip = torch.clamp(y1, 0, max_y) # type: ignore
    dim2 = W
    dim1 = W * H
