...
from monoD.depth_anything_v2.util.transform import Resize
...
class AverageMeter(object):
    ...
    def all_reduce(self):
        ...
        dist.all_reduce(total, dist.ReduceOp.SUM, async_op=False) # type: ignore
        if total.shape[0] > 2:
...
def get_EFP(pred_cameras, image_size, B, S, default_focal=False):
    """
    Converting  cameras to extrinsics, intrinsics matrix
...
def pose_encoding_to_camera(
    ...
    pred_cameras = PerspectiveCamera(focal_length=focal_length,
