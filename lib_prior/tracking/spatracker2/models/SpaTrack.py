...
import utils3d
from tracker3D.spatrack_modules.utils import depth_to_points_colmap, get_nth_visible_time_index
from lib_prior.tracking.spatracker2.models.utils import pose_enc2mat, matrix_to_quaternion, get_track_points, normalize_rgb
import random
...
class SpaTrack2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        if moge_as_base:
            if os.path.exists(backbone_ckpt_dir)==False:
                base_model = MoGeModel.from_pretrained('Ruicheng/moge-2-vits-normal')
            else:
    ...
    def forward_stream(
            ...
            T_org: int = None,
            depth: torch.Tensor|np.ndarray|str|None=None,
            unc_metric_in: torch.Tensor|np.ndarray|str|None=None,
            intrs: torch.Tensor|np.ndarray|str|None=None,
            extrs: torch.Tensor|np.ndarray|str|None=None,
