...
#     Path("reconstructions/{}".format(scene_name)).mkdir(parents=True, exist_ok=True)
...
#     parser.add_argument("--opt_focal", action="store_true", help="use mixed precision")
...
#     image_list = sorted(
#         glob.glob(
#             os.path.join("%s/%s/dense/images" % (args.datapath, scene_name), "*.png")
#         )
#     )

#     # NOTE Mono is inverse depth, but zoe-metric-depth is depth!
#     mono_disp_paths = sorted(
#         glob.glob(os.path.join("%s/%s" % (args.mono_depth_path, scene_name), "*.npy"))
#     )
#     metric_depth_paths = sorted(
#         glob.glob(os.path.join("%s/%s" % (args.metric_depth_path, scene_name), "*.npz"))
#     )
