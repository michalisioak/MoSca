...
def save_tracks(
    ...
    s_mask, d_mask = identify_tracks(
        tracks.permute(1, 0, 2), height=height, width=width, cfg=cfg.identify
    )
    np.savez_compressed(
    ...
    viz_choice = np.random.choice(
        tracks[s_mask].shape[0], min(tracks[s_mask].shape[0], cfg.max_viz_cnt)
    )
    vis.visualize(
        video=img_list.permute(0, 3, 1, 2)[None],
        tracks=tracks[s_mask].permute(1, 0, 2)[None, :, viz_choice, :2],
        visibility=visibility[s_mask].permute(1, 0)[None, :, viz_choice],
        filename=f"static_{name}",
    )
    viz_choice = np.random.choice(
        tracks[d_mask].shape[0], min(tracks[d_mask].shape[0], cfg.max_viz_cnt)
    )
    vis.visualize(
        video=img_list.permute(0, 3, 1, 2)[None],
        tracks=tracks[d_mask].permute(1, 0, 2)[None, :, viz_choice, :2],
        visibility=visibility[d_mask].permute(1, 0)[None, :, viz_choice],
