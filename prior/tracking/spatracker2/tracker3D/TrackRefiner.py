...
import torch
import torch.amp
from .co_tracker.offline import CoTrackerThreeOffline
from .co_tracker.online import CoTrackerThreeOnline
import torch.nn.functional as F
from prior.tracking.spatracker2.utils.visualizer import Visualizer
from prior.tracking.spatracker2.utils.model_utils import sample_features5d
from models.SpaTrackV2.models.blocks import bilinear_sampler
import torch.nn as nn
from .co_tracker.utils import (
    sequence_BCE_loss,
    sequence_loss,
    sequence_prob_loss,
    sequence_loss_xyz,
...
from .spatrack_modules.utils import (
    EfficientUpdateFormer3D,
    posenc,
    key_fr_wprocrustes,
    get_nth_visible_time_index,
)
from .spatrack_modules.ba import (
    extract_static_from_3DTracks,
    ba_pycolmap,
)

from .spatrack_modules.alignment import (
...
class TrackRefiner3D(CoTrackerThreeOffline):
    def __init__(self, args):
        ...
        self.corr_depth_mlp = Mlp(
            in_features=256, hidden_features=256, out_features=256
        )
        self.rel_pos_mlp = Mlp(in_features=75, hidden_features=128, out_features=128)
        self.rel_pos_glob_mlp = Mlp(
            in_features=75, hidden_features=128, out_features=256
        )
        ...
        self.proj_xyz_embed = Mlp(
            in_features=1210 + 50, hidden_features=1110, out_features=1110
        )
        ...
        self.fnet = BasicEncoder(
            input_dim=3, output_dim=self.latent_dim, stride=self.stride
        )
    ...
    def upsample_with_mask(self, inp: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """Upsample flow field [H/P, W/P, 2] -> [H, W, 2] using convex combination"""
        H, W = inp.shape[-2:]
        up_inp = F.unfold(
            inp,
            [self.upsample_kernel_size, self.upsample_kernel_size],
            padding=(self.upsample_kernel_size - 1) // 2,
        )
        up_inp = rearrange(up_inp, "b c (h w) -> b c h w", h=H, w=W)
        up_inp = F.interpolate(
            up_inp, scale_factor=self.upsample_factor, mode="nearest"
        )
        up_inp = rearrange(
            up_inp,
            "b (c i j) h w -> b c (i j) h w",
            i=self.upsample_kernel_size,
            j=self.upsample_kernel_size,
        )
    ...
    def track_from_cam(
        ...
        query_c2w = torch.gather(
            c2w_traj, dim=1, index=query_t[..., None, None].expand(-1, -1, 4, 4)
        )  # B N 4 4
        query_intr = torch.gather(
            intrs, dim=1, index=query_t[..., None, None].expand(-1, -1, 3, 3)
        )  # B N 3 3
        ...
        cam_pts = (
            torch.einsum("bnij,bnj->bni", torch.inverse(query_intr), query_pts)
            * query_d
        )  # B N 3
        # convert to world
        ...
        cam_other_pts_ = torch.einsum(
            "btnij,btnj->btni",
            torch.inverse(c2w_traj[:, :, None].float().repeat(1, 1, N, 1, 1)),
            world_pts[:, None].repeat(1, T, 1, 1),
        )
        cam_depth = cam_other_pts_[..., 2:3]
        cam_other_pts = cam_other_pts_[..., :3] / (
            cam_other_pts_[..., 2:3].abs() + 1e-6
        )
        cam_other_pts = torch.einsum(
            "btnij,btnj->btni",
            intrs[:, :, None].repeat(1, 1, N, 1, 1),
            cam_other_pts[..., :3],
        )
        cam_other_pts[..., 2:] = cam_depth

        if visualize and rgbs is not None:
            viser = Visualizer(
                save_dir=".", grayscale=True, fps=10, pad_value=50, tracks_leave_trace=0
            )
    ...
    def cam_from_track(
        ...
        metric_unc: torch.Tensor,
        vis_est: torch.Tensor,
        track_feat_concat: torch.Tensor,
        tracks_xyz: torch.Tensor,
        query_pts: torch.Tensor,
        depth_unproj: torch.Tensor,
        only_cam_pts=False,
        fixed_cam=False,
        cam_gt=None,
        init_pose=False,
        ...
        if tracks_xyz is not None:
            ...
            tracks_xy[..., 2] = 1
            intr_repeat = intrs[:, :, None].repeat(1, 1, N, 1, 1)
            cam_pts = torch.einsum(
                "bnij,bnj->bni",
                torch.inverse(intr_repeat.view(B * T, N, 3, 3)).float(),
                tracks_xy.view(B * T, N, 3),
            ) * (tracks_z.view(B * T, N, 1).abs())  # B*T N 3
        ...
        with autocast(enabled=False, device_type="cuda"):
            if fixed_cam:
                ...
                world_tracks_init = (
                    torch.einsum(
                        "btij,btnj->btni", c2w_traj_init[:, :, :3, :3], cam_pts
                    )
                    + c2w_traj_init[:, :, None, :3, 3]
                )
                ...
                _, query_world_pts, _ = self.track_from_cam(
                    queries=query_pts, c2w_traj=c2w_traj_init, intrs=intrs_dn
                )
                (
                    world_tracks_static,
                    mask_static,
                ...
                if not self.training:
                    ...
                    campts_update = (
                        torch.einsum(
                            "btij,btnj->btni", self.c2w_est_curr[..., :3, :3], cam_pts
                        )
                        + self.c2w_est_curr[..., None, :3, 3]
                    )
                    # campts_update = cam_pts
                    c2w_traj_init_update = key_fr_wprocrustes(
                        campts_update,
                        graph_matrix,
                        (weight_final * (1 - dyn_prob)).permute(0, 2, 1),
                        vis_est_.permute(0, 1, 3, 2),
                    )
                ...
                intrs_dn[..., 1, :] *= self.factor_y
                _, query_world_pts, _ = self.track_from_cam(
                    query_pts, c2w_traj_init, intrs_dn
                )
                ...
                cam_tracks_static = cam_pts[:, :, mask_static.squeeze(), :][
                    :, :, mask_topk.squeeze(), :
                ]
                cam_tracks_static[..., 2] = depth_unproj.view(B, T, N)[
                    :, :, mask_static.squeeze()
                ][:, :, mask_topk.squeeze()]

                c2w_traj_glob, world_static_refine, intrs_refine = ba_pycolmap(
                    world_tracks_static,
                    intrs,
                    c2w_traj_init,
                    vis_mask_static,
            ...
            cam_pts_refine = (
                torch.einsum(
                    "btij,btnj->btni", w2c_traj_glob[:, :, :3, :3], world_tracks_refined
                )
                + w2c_traj_glob[:, :, None, :3, 3]
            )
            # get the xyz_refine
            # TODO: refiner
            cam_pts4_proj = cam_pts_refine.clone()
            cam_pts4_proj[..., 2] *= torch.sign(cam_pts4_proj[..., 2:3].view(B * T, N))
            xy_refine = torch.einsum(
                "btnij,btnj->btni",
                intrs_refine.view(B, T, 1, 3, 3).repeat(1, 1, N, 1, 1),
                cam_pts4_proj / cam_pts4_proj[..., 2:3].abs(),
            )
        ...
        return (
            ...
            xy_refine,
            world_tracks_init,
            world_tracks_refined,
            c2w_traj_init,
        )

    def extract_img_feat(self, video: torch.Tensor, fmaps_chunk_size=200):
        ...
        if T > fmaps_chunk_size:
            fmaps = []
            C_chunk, H_chunk, W_chunk = None, None, None
            ...
            assert C_chunk is not None and H_chunk is not None and W_chunk is not None
    ...
    def norm_xyz(self, xyz: torch.Tensor):
        ...
        if xyz.ndim == 3:
            ...
            return (xyz - min_pts[None, None, :]) / (max_pts - min_pts)[
                None, None, :
            ] * 2 - 1
        elif xyz.ndim == 4:
            min_pts = self.min_pts
            max_pts = self.max_pts
            return (xyz - min_pts[None, None, None, :]) / (max_pts - min_pts)[
                None, None, None, :
            ] * 2 - 1
        elif xyz.ndim == 5:
            if xyz.shape[2] == 3:
                min_pts = self.min_pts
                max_pts = self.max_pts
                return (xyz - min_pts[None, None, :, None, None]) / (max_pts - min_pts)[
                    None, None, :, None, None
                ] * 2 - 1
            elif xyz.shape[-1] == 3:
                min_pts = self.min_pts
    ...
    def denorm_xyz(self, xyz: torch.Tensor) -> torch.Tensor:
        """
        xyz can be (B T N 3) or (B T 3 H W) or (B N 3)
        """
        min_pts = self.min_pts
        max_pts = self.max_pts
        if xyz.ndim == 3:
            return (xyz + 1) / 2 * (max_pts - min_pts)[None, None, :] + min_pts[
                None, None, :
            ]
        elif xyz.ndim == 4:
            return (xyz + 1) / 2 * (max_pts - min_pts)[None, None, None, :] + min_pts[
                None, None, None, :
            ]
        elif xyz.ndim == 5:
            if xyz.shape[2] == 3:
                return (xyz + 1) / 2 * (max_pts - min_pts)[
                    None, None, :, None, None
                ] + min_pts[None, None, :, None, None]
            elif xyz.shape[-1] == 3:
                return (xyz + 1) / 2 * (max_pts - min_pts)[
                    None, None, None, None, :
                ] + min_pts[None, None, None, None, :]
    ...
    def forward(
        ...
        intrs: torch.Tensor = None,
        traj3d_gt: torch.Tensor = None,
        custom_vid=False,
        vis_gt: torch.Tensor = None,
        prec_fx=None,
        prec_fy=None,
        cam_gt: torch.Tensor = None,
        init_pose=False,
        support_pts_q=None,
        ...
        if (vis_gt is not None) & (queries.shape[1] == T):
            ...
            if aug_visb:
                ...
                first_positive_inds = get_nth_visible_time_index(
                    vis_gt, (number_visible * ratio_rand).long().clamp(min=1, max=T)
                )

                assert (
                    torch.gather(
                        vis_gt, 1, first_positive_inds[:, None, :].repeat(1, T, 1)
                    )
                    < 0
                ).sum() == 0
            else:
            ...
            z_gt_query = torch.diagonal(gather_xyz, dim1=1, dim2=2).permute(0, 2, 1)[
                ..., 2
            ]
        ...
        video_vis = F.interpolate(
            video.clone().view(B * T, 3, video.shape[-2], video.shape[-1]),
            (H_, W_),
            mode="bilinear",
            align_corners=False,
        ).view(B, T, 3, H_, W_)
        ...
        video = F.interpolate(
            video.view(B * T, 3, video.shape[-2], video.shape[-1]),
            (self.model_resolution[0], self.model_resolution[1]),
        ).view(B, T, 3, self.model_resolution[0], self.model_resolution[1])
        _, _, _, H, W = video.shape
        if cache is not None:
            T_cache = cache["fmaps"].shape[0]
            fmaps = self.extract_img_feat(
                video[:, T_cache:], fmaps_chunk_size=fmaps_chunk_size
            )
        ...
        metric_depth = (
            F.interpolate(
                metric_depth.view(B * T, 1, H_, W_),
                (self.model_resolution[0], self.model_resolution[1]),
                mode="nearest",
            )
            .view(B * T, 1, self.model_resolution[0], self.model_resolution[1])
            .clamp(0.01, 200)
        )
        self.metric_unc_org = metric_unc.clone()
        metric_unc = F.interpolate(
            metric_unc.view(B * T, 1, H_, W_),
            (self.model_resolution[0], self.model_resolution[1]),
            mode="nearest",
        ).view(B * T, 1, self.model_resolution[0], self.model_resolution[1])
        ...
        if stage == 2:
            ...
            try:
                ...
                scale_gt, shift_gt = (
                    torch.ones(B * T).to(video.device),
                    torch.zeros(B * T, 3).to(video.device),
                )
        ...
        queries_z = sample_features5d(
            metric_depth.view(B, T, 1, H, W), queries[:, None], interp_mode="nearest"
        ).squeeze(1)
        queries_z_unc = sample_features5d(
            metric_unc.view(B, T, 1, H, W), queries[:, None], interp_mode="nearest"
        ).squeeze(1)

        queries_rgb = sample_features5d(
            video.view(B, T, C, H, W), queries[:, None], interp_mode="nearest"
        ).squeeze(1)
        queries_point_map = sample_features5d(
            point_map.view(B, T, 3, H, W), queries[:, None], interp_mode="nearest"
        ).squeeze(1)
        ...
        if pts_q_3d is not None:
            scale_factor = (
                (
                    pts_q_3d[..., -1].permute(0, 2, 1)
                    / queries_z[:, : pts_q_3d.shape[2], :]
                )
                .squeeze()
                .median()
            )
            queries_z[:, : pts_q_3d.shape[2], :] = (
                pts_q_3d[..., -1].permute(0, 2, 1) / scale_factor
            )
            queries_point_map[:, : pts_q_3d.shape[2], 2:] = (
                pts_q_3d[..., -1].permute(0, 2, 1) / scale_factor
            )

        # normalize the points
        self.min_pts, self.max_pts = (
            queries_point_map.mean(dim=(0, 1)) - 3 * queries_point_map.std(dim=(0, 1)),
            queries_point_map.mean(dim=(0, 1)) + 3 * queries_point_map.std(dim=(0, 1)),
        )
        queries_point_map = self.norm_xyz(queries_point_map)
        queries_point_map_ = (
            queries_point_map.reshape(B, 1, N, 3).expand(B, T, N, 3).clone()
        )
        ...
        if z_gt_query is not None:
            ...
            if traj3d_gt is not None:
                mask_traj_gt = torch.ones_like(
                    queries_z[:, : traj3d_gt.shape[2]]
                ).bool()
            else:
                mask_traj_gt = torch.ones_like(queries_z).bool()

        queries_xyz = torch.cat([queries, queries_z], dim=-1)[:, None].repeat(
            1, T, 1, 1
        )
        if cache is not None:
            cache_T, cache_N = (
                cache["track2d_pred_cache"].shape[0],
                cache["track2d_pred_cache"].shape[1],
            )
            ...
            queries_point_map_[:, :cache_T, :cache_N, :] = self.norm_xyz(
                cache["track3d_pred_cache"][None]
            )
            queries_point_map_[:, cache_T:, :cache_N, :] = self.norm_xyz(
                cache["track3d_pred_cache"][-1:][None]
            )

        if cam_gt is not None:
            q_static_proj, q_xyz_world, q_xyz_cam = self.track_from_cam(
                queries_xyz, cam_gt, intrs, rgbs=video_vis, visualize=False
            )
        ...
        point_map = point_map
        metric_depth_align = F.interpolate(
            metric_depth, scale_factor=0.25, mode="nearest"
        )
        point_map_align = F.interpolate(point_map, scale_factor=0.25, mode="nearest")
        point_map_pyramid.append(
            point_map_align.view(
                B, T, 3, point_map_align.shape[-2], point_map_align.shape[-1]
            )
        )
        for i in range(self.corr_levels - 1):
            ...
            metric_depth_ = metric_depth_align.reshape(
                B * T, 1, metric_depth_align.shape[-2], metric_depth_align.shape[-1]
            )
            metric_depth_ = F.interpolate(
                metric_depth_, scale_factor=0.5, mode="nearest"
            )
            metric_depth_align = metric_depth_.reshape(
                B, T, 1, metric_depth_.shape[-2], metric_depth_.shape[-1]
            )
            # downsample the point map
            point_map_ = point_map_align.reshape(
                B * T, 3, point_map_align.shape[-2], point_map_align.shape[-1]
            )
            point_map_ = F.interpolate(point_map_, scale_factor=0.5, mode="nearest")
            point_map_align = point_map_.reshape(
                B, T, 3, point_map_.shape[-2], point_map_.shape[-1]
            )
        ...
        for i in range(self.corr_levels):
            if cache is not None:
                ...
                track_feat_cached, track_feat_support_cached = (
                    cache["track_feat_pyramid"][i],
                    cache["track_feat_support_pyramid"][i],
                )
                track_feat3d_cached, track_feat_support3d_cached = (
                    cache["track_feat3d_pyramid"][i],
                    cache["track_feat_support3d_pyramid"][i],
                )
                track_point_map_cached, track_point_map_support_cached = (
                    self.norm_xyz(cache["track_point_map_pyramid"][i]),
                    self.norm_xyz(cache["track_point_map_support_pyramid"][i]),
                )
            ...
            if cache is not None:
                ...
                track_point_map = torch.cat(
                    [track_point_map_cached, track_point_map], dim=2
                )
                track_feat_support = torch.cat(
                    [track_feat_support_cached[:, 0], track_feat_support], dim=2
                )
                track_point_map_support = torch.cat(
                    [track_point_map_support_cached[:, 0], track_point_map_support],
                    dim=2,
                )
                track_feat3d = torch.cat([track_feat3d_cached, track_feat3d], dim=2)
                track_feat_support3d = torch.cat(
                    [track_feat_support3d_cached[:, 0], track_feat_support3d], dim=2
                )
        ...
        if init_pose:
            q_init_proj, q_xyz_world, q_xyz_cam = self.track_from_cam(
                queries_xyz, cam_gt, intrs, rgbs=video_vis, visualize=False
            )
        ...
        if init_pose:
            self.c2w_est_curr = cam_gt.to(coords_proj_curr.device).to(
                coords_proj_curr.dtype
            )
        sync_loss = 0
        if stage == 2:
            extra_sparse_tokens = self.scale_shift_tokens[:, :, None, :].repeat(
                B, 1, T, 1
            )
            extra_dense_tokens = self.residual_embedding[None, None].repeat(
                B, T, 1, 1, 1
            )
            xyz_pos_enc = posenc(
                point_map_pyramid[-2].permute(0, 1, 3, 4, 2), min_deg=0, max_deg=10
            ).permute(0, 1, 4, 2, 3)
            extra_dense_tokens = torch.cat(
                [xyz_pos_enc, extra_dense_tokens, fmaps_pyramid[-2]], dim=2
            )
            extra_dense_tokens = rearrange(
                extra_dense_tokens, "b t c h w -> (b t) c h w"
            )
            extra_dense_tokens = self.dense_mlp(extra_dense_tokens)  # type: ignore
            extra_dense_tokens = rearrange(
                extra_dense_tokens, "(b t) c h w -> b t c h w", b=B, t=T
            )
        else:
        ...
        scale_est, shift_est = (
            torch.ones(B, T, 1, 1, device=device),
            torch.zeros(B, T, 1, 3, device=device),
        )
        residual_point = torch.zeros(
            B,
            T,
            3,
            self.model_resolution[0] // self.stride,
            self.model_resolution[1] // self.stride,
            device=device,
        )

        for it in range(iters):
            # query points scale and shift
            scale_est_query = torch.gather(
                scale_est, dim=1, index=queries[:, :, None, :1].long()
            )
            shift_est_query = torch.gather(
                shift_est,
                dim=1,
                index=queries[:, :, None, :1].long().repeat(1, 1, 1, 3),
            )
            ...
            for i in range(self.corr_levels):
                ...
                residual_point_i = F.interpolate(
                    residual_point.view(
                        B * T, 3, residual_point.shape[-2], residual_point.shape[-1]
                    ),
                    size=(
                        point_map_pyramid[i].shape[-2],
                        point_map_pyramid[i].shape[-1],
                    ),
                    mode="nearest",
                )
                point_map_pyramid_i = (
                    (
                        self.denorm_xyz(point_map_pyramid[i]) * scale_est[..., None]
                        + shift_est.permute(0, 1, 3, 2)[..., None]
                        + residual_point_i.view(
                            B,
                            T,
                            3,
                            point_map_pyramid[i].shape[-2],
                ...
                corr_point_map = self.get_correlation_feat(
                    self.norm_xyz(point_map_pyramid_i),
                    coords_proj_curr / 2**i,
                    radius=self.corr3d_radius,
                )
                ...
                # NOTE: update the point map
                track_point_map_support_pyramid_i = (
                    (
                        self.denorm_xyz(track_point_map_support_pyramid[i])
                        * scale_est_query.view(B, 1, 1, N, 1)
                        + shift_est_query.view(B, 1, 1, N, 3)
                    )
                    .clone()
                    .detach()
                )
                ...
                with torch.no_grad():
                    rel_pos_query_ = (
                        track_point_map_support
                        - track_point_map_support[
                            :, :, self.corr3d_radius, self.corr3d_radius, :
                        ][..., None, None, :]
                    )
                    rel_pos_target_ = corr_point_map - coords_xyz_init.view(
                        B, T, N, 1, 1, 3
                    )
                    # select the top 9 points
                    rel_pos_query_idx = (
                        rel_pos_query_.norm(dim=-1)
                        .view(B, N, -1)
                        .topk(K_level + 1, dim=-1, largest=False)[1][..., 1:, None]
                    )
                    rel_pos_target_idx = (
                        rel_pos_target_.norm(dim=-1)
                        .view(B, T, N, -1)
                        .topk(K_level + 1, dim=-1, largest=False)[1][..., 1:, None]
                    )
                    rel_pos_query_ = torch.gather(
                        rel_pos_query_.view(B, N, -1, 3),
                        dim=-2,
                        index=rel_pos_query_idx.expand(B, N, K_level, 3),
                    )
                    rel_pos_target_ = torch.gather(
                        rel_pos_target_.view(B, T, N, -1, 3),
                        dim=-2,
                        index=rel_pos_target_idx.expand(B, T, N, K_level, 3),
                    )
                ...
                with torch.no_grad():
                    ...
                    track_feat_support_ = torch.gather(
                        track_feat_support_,
                        dim=-2,
                        index=rel_pos_query_idx.expand(B, N, K_level, 128),
                    )
                    queried_feat = torch.cat(
                        [rel_pos_query, track_feat_support_], dim=-1
                    )
                    corr_feat_ = rearrange(
                        corr_point_feat, "b t n r k c -> b t n (r k) c", t=T, n=N, b=B
                    )
                    corr_feat_ = torch.gather(
                        corr_feat_,
                        dim=-2,
                        index=rel_pos_target_idx.expand(B, T, N, K_level, 128),
                    )
                ...
                queried_feat = repeat(
                    queried_feat, "b n k c -> b t n k c", k=K_level, t=T, n=N, b=B
                )
                corr_depth_emb = self.corr_transformer[0](
                    queried_feat.reshape(B * T * N, -1, 128),
                    target_feat.reshape(B * T * N, -1, 128),
                    target_rel_pos=rel_pos_target.reshape(B * T * N, -1, 128),
                )
                corr_depth_emb = rearrange(
                    corr_depth_emb, "(b t n) 1 c -> b t n c", t=T, n=N, b=B
                )
                corr_depth_emb = self.corr_depth_mlp(corr_depth_emb)
                valid_mask = (
                    self.denorm_xyz(coords_xyz_init).view(B, T, N, -1)[..., 2:3] > 0
                )
            ...
            with torch.no_grad():
                ...
                queries_xyz_i[..., -1] = (
                    queries_xyz_i[..., -1] * scale_est_query.view(B, 1, N)
                    + shift_est_query.view(B, 1, N, 3)[..., 2]
                )
                _, _, q_xyz_cam = self.track_from_cam(
                    queries_xyz_i, self.c2w_est_curr, intrs, rgbs=None, visualize=False
                )
                q_xyz_cam = self.norm_xyz(q_xyz_cam)

            query_t = queries[:, None, :, :1].repeat(B, T, 1, 1)
            q_xyz_cam = torch.cat([query_t / T, q_xyz_cam], dim=-1)
            T_all = torch.arange(T, device=device)[None, :, None, None].repeat(
                B, 1, N, 1
            )
            current_xyzt = torch.cat(
                [T_all / T, coords_xyz_init.view(B, T, N, -1)], dim=-1
            )
            ...
            dynamic_prob_curr = (
                torch.sigmoid(dynamic_prob[..., None])
                .mean(dim=1, keepdim=True)
                .repeat(1, T, 1, 1)
            )
            # embed the confidence and dynamic probability
            rel_pos_query_glob = torch.cat(
                [rel_pos_query_glob, confidence_curr, dynamic_prob_curr], dim=-1
            )
            ...
            if scale_shift_out is not None:
                ...
                up_mask = self.upsample_transformer(
                    extra_dense_tokens.mean(dim=1), res_low
                )
            ...
            pro_analysis_w = pro_analysis_w + delta_pro_analysis_w[..., 0].permute(
                0, 2, 1
            )
            # update the depth
            ...
            denorm_delta_depth = (
                self.denorm_xyz(coords_xyz + delta_xyz) - self.denorm_xyz(coords_xyz)
            )[..., 2:3]
            ...
            dynamic_prob_curr = torch.sigmoid(dynamic_prob.detach()) * torch.sigmoid(
                pro_analysis_w
            )
            mask_out = (
                (coords_append[..., 0] < W_)
            ...
            if query_no_BA:
                dynamic_prob_curr[:, :, :ba_len] = torch.ones_like(
                    dynamic_prob_curr[:, :, :ba_len]
                )
            point_map_org_i = scale_est.view(
                B * T, 1, 1, 1
            ) * point_map_org.clone().detach() + shift_est.view(B * T, 3, 1, 1)
            ...
            depth_unproj_neg = self.get_correlation_feat(
                point_map_org_i.view(
                    B, T, 3, point_map_org_i.shape[-2], point_map_org_i.shape[-1]
                ),
                coords_append[..., :2].view(B * T, N, 2),
                radius=self.corr3d_radius,
            )[..., 2]
            depth_diff = (
                depth_unproj_neg.view(B, T, N, -1) - coords_append[..., 2:]
            ).abs()
            idx_neg = torch.argmin(depth_diff, dim=-1)
            depth_unproj = depth_unproj_neg.view(B, T, N, -1)[
                torch.arange(B)[:, None, None, None],
                torch.arange(T)[None, :, None, None],
                torch.arange(N)[None, None, :, None],
                idx_neg.view(B, T, N, 1),
            ].view(B * T, N)

            unc_unproj = bilinear_sampler(
                self.metric_unc_org,
                coords_append[..., :2].view(B * T, N, 1, 2),
                mode="nearest",
            )[:, 0, :, 0].detach()
            ...
            replace_mask = (depth_unproj.view(B, T, N) > 0.0) * (
                vis_est > 0.5
            )  # * (conf_est>0.5)
            ...
            depth_ddt2 = (
                depth_rel[:, 2:, :] - 2 * depth_rel[:, 1:-1, :] + depth_rel[:, :-2, :]
            )
            jitter_mask = torch.zeros_like(depth_rel, dtype=torch.bool)
            if depth_ddt2.abs().max() > 0:
                thre2 = torch.quantile(
                    depth_ddt2.abs()[depth_ddt2.abs() > 0], replace_ratio
                )
                jitter_mask[:, 1:-1, :] = depth_ddt2.abs() < thre2
                thre1 = torch.quantile(
                    depth_ddt1.abs()[depth_ddt1.abs() > 0], replace_ratio
                )
            ...
            coords_append[..., 2][replace_mask] = depth_unproj.view(B, T, N)[
                replace_mask
            ]
            coords_xyz_unproj = self.cam_from_track(
                coords_append.clone(), intrs_org, only_cam_pts=True
            )
            ...
            (
                ...
                dynamic_prob_curr,
                queries_z_unc,
                conf_est * vis_est * mask_out.float(),
                track_feat_concat=x_depth,
                tracks_xyz=coords_xyz_append_refine,
                init_pose=init_pose,
                query_pts=queries_xyz_i,
                fixed_cam=fixed_cam,
                depth_unproj=depth_unproj,
                cam_gt=cam_gt,
            )
            ...
            coords_proj = torch.einsum(
                "btij,btnj->btni",
                intrs_org,
                coords_xyz_de / coords_xyz_de[..., 2:3].abs(),
            )[..., :2]
            ...
            sync_loss += (
                vis_est.detach()[..., None]
                * (coords_proj_curr - coords_proj).norm(dim=-1, keepdim=True)
                * (1 - mask_nan[..., None].float())
            ).mean()
            ...
            fix_cam_pts = self.cam_from_track(fix_cam_est, intrs_org, only_cam_pts=True)
        ...
        if stage != 0:
            ...
            all_coords_xyz_predictions.append(
                [coord_xyz for coord_xyz in coords_xyz_preds]
            )
            ...
            all_world_tracks_predictions.append(
                [world_tracks for world_tracks in world_tracks_pred]
            )
            all_world_tracks_refined_predictions.append(
                [
                    world_tracks_refined
                    for world_tracks_refined in world_tracks_refined_pred
                ]
            )
        ...
        if train_data is not None:
            ...
            if cam_gt is not None:
                gt_world_pts = (
                    torch.einsum("btij,btnj->btni", cam_gt[..., :3, :3], gt_cam_pts)
                    + cam_gt[..., None, :3, 3]
                )  # B T N 3
            else:
                gt_world_pts = (
                    torch.einsum(
                        "btij,btnj->btni", self.c2w_est_curr[..., :3, :3], gt_cam_pts
                    )
                    + self.c2w_est_curr[..., None, :3, 3]
                )  # B T N 3
                # update the query points with scale and shift
            ...
            if traj3d_gt is not None:
                ret_loss = self.loss(
                    ...
                    fix_cam_pts=fix_cam_pts,
                    video_vis=video_vis,
                    stage=stage,
                    gt_world_pts=gt_world_pts,
                    mask_traj_gt=mask_traj_gt,
                    intrs=intrs_org,
                    custom_vid=custom_vid,
                    valid_only=valid_only,
                    c2w_ests=c2w_ests,
                    point_map_preds=point_map_preds,
                    points_map_gt=points_map_gt,
                    metric_unc=metric_unc,
                    scale_est=scale_est,
                    shift_est=shift_est,
                    point_map_org_train=point_map_org_train,
                )
            else:
                ret_loss = self.loss(
                    train_data,
        ...
        color_pts = torch.cat(
            [pts3d_cam_pred[-1], queries_rgb[:, None].repeat(1, T, 1, 1)], dim=-1
        )
        ...
        ret = {
            "preds": coord_preds[-1],
            "vis_pred": vis_pred_out,
            "conf_pred": torch.sigmoid(confidence_preds[-1]),
            "cam_pred": self.c2w_est_curr,
            "loss": ret_loss,
        }
        ...
        cache = {
            ...
            "track_feat_support3d_pyramid": [
                track_feat_support3d_pyramid[i].detach()
                for i in range(len(track_feat_support3d_pyramid))
            ],
            "track_point_map_support_pyramid": [
                self.denorm_xyz(track_point_map_support_pyramid[i].detach())
                for i in range(len(track_point_map_support_pyramid))
            ],
            "track_feat3d_pyramid": [
                track_feat3d_pyramid[i].detach()
                for i in range(len(track_feat3d_pyramid))
            ],
            "track_point_map_pyramid": [
                self.denorm_xyz(track_point_map_pyramid[i].detach())
                for i in range(len(track_point_map_pyramid))
            ],
            "track_feat_pyramid": [
                track_feat_pyramid[i].detach() for i in range(len(track_feat_pyramid))
            ],
            "track_feat_support_pyramid": [
                track_feat_support_pyramid[i].detach()
                for i in range(len(track_feat_support_pyramid))
            ],
            "track2d_pred_cache": coord_preds[-1][0].clone().detach(),
            "track3d_pred_cache": pts3d_cam_pred[-1][0].clone().detach(),
        }
        # NOTE: update the point map
        point_map_org = scale_est.view(B * T, 1, 1, 1) * point_map_org + shift_est.view(
            B * T, 3, 1, 1
        )
        point_map_org_refined = point_map_org
        return (
            ret,
            torch.sigmoid(dynamic_prob_preds[-1]) * queries_z_unc[:, None, :, 0],
            coord_preds[-1],
            color_pts,
            intrs_org,
            point_map_org_refined,
            cache,
        )
    ...
    def track_d2_loss(self, tracks3d, stride=[1, 2, 3], dyn_prob=None, mask=None):
        ...
        for i, s_ in enumerate(stride):
            ...
            t_diff_tracks3d = (
                tracks3d_stride[:, 1:, :, :] - tracks3d_stride[:, :-1, :, :]
            )
    ...
    def loss(
        self,
        train_data,
        traj3d_gt: torch.Tensor,
        vis_gt: torch.Tensor,
        static_tracks_gt: torch.Tensor | None,
        cam_gt: torch.Tensor,
        z_unc: torch.Tensor,
        q_xyz_world: torch.Tensor,
        q_static_proj: torch.Tensor,
        # anchor_loss=0,
        valid_only: torch.Tensor,
        gt_world_pts: torch.Tensor,
        mask_traj_gt: torch.Tensor,
        intrs: torch.Tensor,
        c2w_ests: torch.Tensor,
        custom_vid: torch.Tensor,
        video_vis: torch.Tensor,
        fix_cam_pts: torch.Tensor,
        point_map_preds: torch.Tensor,
        points_map_gt: torch.Tensor,
        metric_unc: torch.Tensor,
        scale_est: torch.Tensor,
        shift_est: torch.Tensor,
        point_map_org_train: torch.Tensor,
        stage=0,
    ):
        """
        ...
        (
            ...
            dynamic_prob_predictions,
            camera_predictions,
            cam_pts_predictions,
            world_tracks_predictions,
            world_tracks_refined_predictions,
            scale_ests,
            shift_ests,
            valid_mask,
        ) = train_data
        ...
        if (stage == 2) and self.training:
            ...
            self.metric_unc_org[:, 0] = (
                self.metric_unc_org[:, 0]
                * (points_map_gt.norm(dim=-1) > 0).float()
                * (self.metric_unc_org[:, 0] > 0.5).float()
            )
            if not (self.scale_gt == torch.ones(B * T).to(self.scale_gt.device)).all():
                ...
                for i_scale in range(len(scale_ests[0])):
                    scale_loss += (
                        0.8 ** (len(scale_ests[0]) - i_scale - 1)
                        * 10
                        * (scale_gt - scale_re * scale_ests[0][i_scale].view(-1))
                        .abs()
                        .mean()
                    )
                    shift_loss += (
                        0.8 ** (len(shift_ests[0]) - i_scale - 1)
                        * 10
                        * (shift_gt - scale_re * shift_ests[0][i_scale].view(-1, 3))
                        .abs()
                        .mean()
                    )
            else:
        ...
        if len(point_map_preds) > 0:
            point_map_loss = 0.0
            point_map_preds_i = None
            for i in range(len(point_map_preds)):
                point_map_preds_i = point_map_preds[i]
                point_map_preds_i = rearrange(
                    point_map_preds_i, "b t c h w -> (b t) c h w", b=B, t=T
                )
                # base_loss = (
                #     (self.pred_points - points_map_gt).norm(dim=-1)
                #     * self.metric_unc_org[:, 0]
                # ).mean()
                point_map_loss_i = (
                    (point_map_preds_i - points_map_gt.permute(0, 3, 1, 2)).norm(dim=1)
                    * self.metric_unc_org[:, 0]
                ).mean()
            ...
            if scale_loss == 0.0:
                assert point_map_preds_i is not None
        ...
        in_view_mask = (
            (q_static_proj[..., 0] > 0)
            & (q_static_proj[..., 0] < W_org)
            & (q_static_proj[..., 1] > 0)
            & (q_static_proj[..., 1] < H_org)
        )
        dyn_mask_final = (coord_predictions[0][-1] - q_static_proj)[..., :2].norm(
            dim=-1
        ) * in_view_mask
        ...
        for iter_, cam_pred_i in enumerate(camera_predictions[0]):
            ...
            pts_i_world_refined = (
                torch.einsum("btij,btnj->btni", cam_gt[..., :3, :3], coords_xyz_i_world)
                + cam_gt[..., None, :3, 3]
            )  # B T N 3
            ...
            #     ).norm(dim=-1)
            #     * vis_i[:, :, :N_gt]
            # ).sum(dim=1) / vis_i[:, :, :N_gt].sum(dim=1)
            ...
            pts_loss_refined = (
                (q_xyz_world[:, None, ...] - pts_i_world_refined).norm(dim=-1)
                * static_mask[:, None, :]
            ).sum() / static_mask.sum()  # - 0.1*(1-dyn_prob_i[:,None,:N_gt]).log()
            vis_logits_final = vis_predictions[0][-1].detach()
            vis_final = (
                torch.sigmoid(vis_logits_final) + 0.2 > 0.5
            )  # more strict for visible points
            dyn_vis_mask = dyn_mask * vis_final * (fix_cam_pts[..., 2] > 0.1)
            pts_loss_dynamic = (
                (fix_cam_pts - coords_xyz_i_world).norm(dim=-1)
                * dyn_vis_mask[:, None, :]
            ).sum() / dyn_vis_mask.sum()

            # pts_loss_refined = 0
            if traj3d_gt is not None:
                tap_traj = (
                    (gt_world_pts[:, :-1, ...] - gt_world_pts[:, 1:, ...])
                    .norm(dim=-1)
                    .sum(dim=1)[..., :N_gt_mask]
                )
                mask_dyn = tap_traj > 0.5
                if mask_traj_gt.sum() > 0:
                    dyn_loss_i = 20 * balanced_binary_cross_entropy(
                        dyn_prob_i_logits[:, :N_gt_mask][mask_traj_gt.squeeze(-1)],
                        mask_dyn.float()[mask_traj_gt.squeeze(-1)],
                    )
                else:
                    dyn_loss_i = 0
            else:
                dyn_loss_i = 10 * balanced_binary_cross_entropy(
                    dyn_prob_i_logits, dyn_mask_final.float()
                )
            ...
            out_of_view_mask = (
                (coords_i[..., 0] < 0)
                | (coords_i[..., 0] > self.image_size[1])
                | (coords_i[..., 1] < 0)
                | (coords_i[..., 1] > self.image_size[0])
            )
            vis_loss_out_of_view = (
                vis_i_train[out_of_view_mask].sum() / out_of_view_mask.sum()
            )
            ...
            #             (
            #                 gt_world_pts
            #                 - pts_i_world_refined[:, :, : gt_world_pts.shape[2], ...]
            #             ).norm(dim=-1)
            #         )[..., :N_gt_mask]
            #         * mask_traj_gt.permute(0, 2, 1)
            #     ).sum() / mask_traj_gt.sum()
            ...
            in_view_mask_large = (
                (q_static_proj[..., 0] > -50)
                & (q_static_proj[..., 0] < W_org + 50)
                & (q_static_proj[..., 1] > -50)
                & (q_static_proj[..., 1] < H_org + 50)
            )
            static_vis_mask = (
                (q_static_proj[..., 2] > 0.05).float()
                * static_mask[:, None, :]
                * in_view_mask_large
            )
            xyz_loss = (coord_predictions[0][iter_] - q_static_proj).abs()[
                ..., :2
            ].norm(dim=-1) * static_vis_mask
            ...
            # temporal loss
            # t_loss = self.track_d2_loss(
            #     pts_i_world_refined, [1, 2, 3], dyn_prob=dyn_prob_i, mask=mask
            # )
            R_err = (
                (cam_pred_i[..., :3, :3] - cam_gt[..., :3, :3]).abs().sum(dim=-1).mean()
            )
            if self.stage == 1:
                cam_loss += 0.8 ** (len(camera_predictions[0]) - iter_ - 1) * (
                    10 * t_err
                    + 500 * R_err
                    + 20 * pts_loss_refined
                    + 10 * xyz_loss
                    + 20 * pts_loss_dynamic
                    + 10 * vis_loss_out_of_view
                )  # + 5*(pts_loss + pts_loss_refined + world_pts_loss) + t_loss)
            elif self.stage == 3:
                cam_loss += 0.8 ** (len(camera_predictions[0]) - iter_ - 1) * (
                    10 * t_err + 500 * R_err + 10 * vis_loss_out_of_view
                )  # + 5*(pts_loss + pts_loss_refined + world_pts_loss) + t_loss)
            else:
        ...
        if (torch.isnan(traj3d_gt).any() or traj3d_gt.abs().max() > 2000) and (
            not custom_vid
        ):
            return (
                cam_loss,
        ...
        seq_loss_all = sequence_loss(
            ...
            loss_only_for_visible=False if not custom_vid else True,
            z_unc=z_unc,
            mask_traj_gt=mask_traj_gt,
        )
        ...
        blink_mask = (
            mask_vis[:, :-1, :] * mask_vis[:, 1:, :]
        )  # first derivative   B (T-1) N
