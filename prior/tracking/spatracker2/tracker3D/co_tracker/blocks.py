...
from prior.tracking.spatracker2.utils.model_utils import bilinear_sampler
...
class CorrBlock:
    ...
    def corr(self, targets: torch.Tensor):
        ...
        for i, fmaps in enumerate(self.fmaps_pyramid):
            *_, H, W = fmaps.shape
            ...
            if self.multiple_track_feats:
                assert targets_split is not None
...
class Attention(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, context: torch.Tensor | None = None, attn_bias=None
    ):
        B, N1, C = x.shape
        h = self.heads
