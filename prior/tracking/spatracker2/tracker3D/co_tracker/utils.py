...
class Attention(nn.Module):
    def __init__(
        self, query_dim, context_dim: int, num_heads=8, dim_head=48, qkv_bias=False
    ):
        super().__init__()
    ...
    def forward(self, x, context, attn_bias=None, flash=True):
        B, N1, C = x.shape
        h = self.heads
        ...
        if not flash:
            ...
            try:
                ...
                with torch.backends.cuda.sdp_kernel(
                    ...
                    x = (
                        F.scaled_dot_product_attention(*input_args)
                        .permute(0, 2, 1, 3)
                        .reshape(B, N1, -1)
                    )  # type: ignore
...
class EfficientUpdateFormer(nn.Module):
    ...
    def forward(
        ...
        patch_feat: torch.Tensor | None = None,
    ):
        tokens: torch.Tensor = self.input_transform(input_tensor)
        ...
        if patch_feat is None:
            ...
            patch_feat_proj: torch.Tensor = self.patch_proj(patch_feat.detach())
            tokens = torch.cat([tokens, patch_feat_proj], dim=1)
            self.num_virtual_tracks = patch_feat_proj.shape[1]

        _, N, _, _ = tokens.shape
        j = 0
        for i in range(len(self.time_blocks)):
            time_tokens: torch.Tensor = tokens.contiguous().view(
                B * N, T, -1
            )  # B N T C -> (B N) T C
            time_tokens = torch.as_tensor(checkpoint(self.time_blocks[i], time_tokens))
            ...
            if (
                ...
                virtual_tokens = torch.as_tensor(
                    checkpoint(self.space_virtual_blocks[j], virtual_tokens)
                )

                point_tokens = torch.as_tensor(
                    checkpoint(
...
def balanced_binary_cross_entropy(
    ...
    loss = F.binary_cross_entropy_with_logits(
        logits, targets, pos_weight=pos_weight + pos_bias, reduction=reduction
    )
    if mask is not None:
    ...
    for j in range(len(flow_gt)):
        ...
        for i in range(n_predictions):
            ...
            if add_huber_loss:
                ...
                if flow_gt[j][..., 2].abs().max() != 0:
                    ...
                    if mask_traj_gt is not None:
                        track_z_loss = (
                            (flow_pred - flow_gt[j])[..., 2].abs()
                            * mask_traj_gt.permute(0, 2, 1)
                        ).sum() / (mask_traj_gt.sum(dim=1) + 1e-6)
                else:
                    track_z_loss = 0
            ...
            valid_ = valids[j].clone()[
                :, :, : flow_gt[j].shape[2]
            ]  # Ensure valid_ has the same shape as i_loss
            ...
            flow_loss += i_weight * (
                reduce_masked_mean(i_loss, valid_)
                + track_z_loss
                + 10 * reduce_masked_mean(i_loss, valid_ * vis[j])
            )
            # if flow_loss > 5e2:
...
def sequence_loss_xyz(
    ...
    for j in range(len(flow_gt)):
        ...
        for i in range(n_predictions):
            ...
            flow_pred = flow_preds[j][i][:, :, : flow_gt[j].shape[2]]
            flow_gt_ = flow_gt[j]
            flow_gt_one = torch.cat(
                [flow_gt_[..., :2], torch.ones_like(flow_gt_[:, :, :, :1])], dim=-1
            )
            flow_gt_cam = torch.einsum(
                "btsc,btnc->btns", torch.inverse(intrs), flow_gt_one
            )
            ...
            if add_huber_loss:
                ...
                i_loss = (flow_pred - flow_gt_cam).norm(
                    dim=-1, keepdim=True
                )  # B, S, N, 2
...
def sequence_BCE_loss(vis_preds, vis_gts, mask=None):
    ...
    for j in range(len(vis_preds)):
        ...
        for i in range(n_predictions):
            ...
            if mask is not None:
                vis_loss = balanced_binary_cross_entropy(
                    vis_preds[j][i][..., :N_gt],
                    vis_gts[j],
                    mask=mask[j],
                    reduction="none",
                ...
                vis_loss = balanced_binary_cross_entropy(
                    vis_preds[j][i][..., :N_gt], vis_gts[j]
                ) + focal_loss(vis_preds[j][i][..., :N_gt], vis_gts[j])
...
def sequence_prob_loss(
    ...
    for j in range(len(tracks)):
        ...
        for i in range(n_predictions):
            ...
            err = torch.sum(
                (tracks[j][i].detach()[:, :, :N_gt, :2] - target_points[j][..., :2])
                ** 2,
                dim=-1,
            )
            valid = (err <= expected_dist_thresh**2).float()
            logprob = balanced_binary_cross_entropy(
                confidence[j][i][..., :N_gt], valid, reduction="none"
            )
...
def sequence_dyn_prob_loss(
    ...
    for j in range(len(tracks)):
        ...
        for i in range(n_predictions):
            ...
            logprob = balanced_binary_cross_entropy(
                confidence[j][i].mean(dim=1), valid, reduction="none"
            )
            # logprob *= visibility[j]
...
class NeighborTransformer(nn.Module):
    def __init__(self, dim: int, num_heads: int, head_dim: int, mlp_ratio: float):
        ...
        self.aggr1 = Attention(
            dim, context_dim=dim, num_heads=num_heads, dim_head=head_dim
        )
        self.aggr2 = Attention(
            dim, context_dim=dim, num_heads=num_heads, dim_head=head_dim
        )
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        def forward_chunk(x: torch.Tensor, y: torch.Tensor):
            new_x = self.xblock1_2(x, y)
            new_y = self.xblock2_1(y, x)
            out1 = self.aggr1(
                repeat(self.output_token_1, "n d -> b n d", b=x.shape[0]), context=new_x
            )
            out2 = self.aggr2(
                repeat(self.output_token_2, "n d -> b n d", b=x.shape[0]), context=new_y
            )
            return out1 + out2

        result = checkpoint(forward_chunk, x, y)
...
class CorrPointformer(nn.Module):
    ...
    def forward(
        self, query: torch.Tensor, target: torch.Tensor, target_rel_pos: torch.Tensor
    ) -> torch.Tensor:
        def forward_chunk(query, target, target_rel_pos):
            ...
            out = self.aggr(
                new_query, target + target_rel_pos
            )  # (potential delta xyz)  (target - center)
            out = self.out_proj(out)
            return out

        result = checkpoint(forward_chunk, query, target, target_rel_pos)
        return torch.as_tensor(result)
