...
from prior.tracking.spatracker2.utils.embeddings import (
    get_1d_sincos_pos_embed_from_grid,
)

from .blocks import Mlp, BasicEncoder
from .utils import EfficientUpdateFormer
...
class CoTrackerThreeBase(nn.Module):
    ...
    def interpolate_time_embed(self, x, t):
        ...
        time_emb = F.interpolate(
            torch.as_tensor(time_emb).permute(0, 2, 1), size=t, mode="linear"
        ).permute(0, 2, 1)
...
class CoTrackerThreeOnline(CoTrackerThreeBase):
    ...
    def init_video_online_processing(self):
        self.online_ind = 0
        self.online_track_feat: list[torch.Tensor]
        self.online_track_support: list[torch.Tensor]
        self.online_coords_predicted = None
        self.online_vis_predicted: torch.Tensor
        self.online_conf_predicted: torch.Tensor
    ...
    def forward_window(
        ...
        for it in range(iters):
            ...
            x = x + torch.as_tensor(self.interpolate_time_embed(x, S))
    ...
    def forward(
        ...
        if (not is_train) and (T > fmaps_chunk_size):
            ...
            fmaps = torch.cat(fmaps_int, dim=1).reshape(-1, C_chunk, H_chunk, W_chunk)
        else:
            fmaps: torch.Tensor = self.fnet(video.reshape(-1, C_, H, W))
