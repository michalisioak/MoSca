...
class Visualizer:
    ...
    def visualize(
        ...
        segm_mask: torch.Tensor,  # (B,1,H,W)
        filename: str = "video",
        ...
        if video_depth is not None:
            ...
            video_depth = [
                cv2.applyColorMap(video_depth[0, i, 0], cv2.COLORMAP_INFERNO)
        ...
        if save_video:
            ...
            if video_depth is not None:
                self.save_video(
                    video_depth, filename=filename + "_depth", writer=writer, step=step
                )
        return res_video
    ...
    def draw_tracks_on_video(
        ...
        rigid_part: torch.Tensor | None = None,
    ):
        ...
        assert D == 2
        assert C == 3
        rgbs = video[0].permute(0, 2, 3, 1).byte().detach().cpu().numpy()  # S, H, W, C
        ...
        if self.mode == "optical_flow":
            vector_colors = flow_vis.flow_to_color(
                tracks_np - tracks_np[query_frame][None]
            )
        ...
        if self.tracks_leave_trace != 0:
            for t in range(1, T):
                ...
                curr_tracks = tracks_np[first_ind : t + 1]
            ...
            for i in range(N):
                ...
                if coord[0] != 0 and coord[1] != 0:
                    if not compensate_for_camera_motion or (
                        ...
                        if rigid_part is not None and color_map is not None:
                            color: tuple[int, int] = color_map[
                                rigid_part.squeeze()[i].item()
                            ]
                            cv2.circle(
                                res_video[t],
                                coord,
        ...
        for t in range(T):
            for i in range(N):
                ...
                if gt_tracks_i[0] > 0 and gt_tracks_i[1] > 0:
                    ...
                    coord_y = (
                        int(gt_tracks_i[0]) + length,
                        int(gt_tracks_i[1]) + length,
                    )
                    coord_x = (
                        int(gt_tracks_i[0]) - length,
                        int(gt_tracks_i[1]) - length,
