...
def nearest_sample2d(im, x, y, return_inbounds=False):
    ...
    x0_clip = torch.clamp(x0, min=0, max=max_x.item())
    x1_clip = torch.clamp(x1, min=0, max=max_x.item())
    y0_clip = torch.clamp(y0, min=0, max=max_y.item())
    y1_clip = torch.clamp(y1, min=0, max=max_y.item())
    dim2 = W
    dim1 = W * H
    ...
    output = (
        torch.stack([i_y0_x0, i_y0_x1, i_y1_x0, i_y1_x1], dim=-1)
        .gather(-1, max_idx[..., None, None].repeat(1, 1, C, 1))
        .squeeze(-1)
    )
...
def bilinear_sampler(
    input, coords, align_corners=True, padding_mode="border", interp_mode="bilinear"
):
