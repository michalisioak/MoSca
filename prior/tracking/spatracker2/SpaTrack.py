...
from tracker3D.TrackRefiner import TrackRefiner3D
from .utils.model_utils import sample_features5d
import utils3d
from tracker3D.spatrack_modules.utils import (
    depth_to_points_colmap,
...
class SpaTrack2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.base_model = MoGeModel.from_pretrained(cfg.moge_model)
        ...
        self.make_paras_trainable = partial(
            self.make_paras_trainable,
            mode=ft_cfg["mode"],
    ...
    def ProcVid(self, x: torch.Tensor):
        """
        ...
        num_types = x.dtype
        x = normalize_rgb(x, input_size=self.cfg.resolution)
        x = x.to(num_types)
    ...
    def forward_stream(
        ...
        unc_metric_in: torch.Tensor | None = None,
        intrs: torch.Tensor | None = None,
        extrs: torch.Tensor | None = None,
        queries_3d: torch.Tensor | None = None,
        window_len: int = 16,
        overlap_len: int = 4,
        full_point: bool = False,
        track2d_gt: torch.Tensor | None = None,
        fixed_cam: bool = False,
        query_no_BA: bool = False,
        stage: int = 0,
        support_frame: int = 0,
        replace_ratio: float = 0.6,
        annots_train: Dict | None = None,
        iters_track=4,
        **kwargs,
    ):
        ...
        if annots_train is not None:
            ...
            assert (
                torch.gather(vis_gt, 1, first_positive_inds[:, None, :].repeat(1, T, 1))
                < 0
            ).sum() == 0

            first_positive_inds = first_positive_inds.long()
            gather = torch.gather(
                annots_train["traj_3d"][..., :2],
                1,
                first_positive_inds[:, :, None, None].repeat(1, 1, N, 2),
            )
            xys = torch.diagonal(gather, dim1=1, dim2=2).permute(0, 2, 1)
            queries = torch.cat([first_positive_inds[:, :, None], xys], dim=-1)[0].cpu()
        ...
        if T < window_len:
            ...
            video_unf = video.unfold(0, window_len, step_slide).permute(
                0, 4, 1, 2, 3
            )  # [B, S, C, H, W]
            if depth is not None:
            ...
            if extrs is not None:
                ...
                extrs_unf = None
            if unc_metric_in is not None:
                unc_metric_unf = unc_metric_in.unfold(
                    0, window_len, step_slide
                ).permute(0, 3, 1, 2)
            else:
                unc_metric_unf = None
        ...
        for i in range(B):
            ...
            queries_new_mask = (sort_query[..., 0] < i * step_slide + window_len) * (
                sort_query[..., 0] >= (i * step_slide + overlap_len if i > 0 else 0)
            )
            if queries_3d is not None:
            ...
            if i > 0:
                overlap2d = track2d_pred[
                    i * step_slide : (i + 1) * step_slide, :queries_len, :
                ]
                overlapvis = vis_pred[
                    i * step_slide : (i + 1) * step_slide, :queries_len, :
                ]
                overlapconf = conf_pred[
                    i * step_slide : (i + 1) * step_slide, :queries_len, :
                ]
                overlap_query = (overlapvis * overlapconf).max(dim=0)[1][None, ...]
                overlap_xy = torch.gather(overlap2d, 0, overlap_query.repeat(1, 1, 2))
                overlap_d = torch.gather(overlap2d, 0, overlap_query.repeat(1, 1, 3))[
                    ..., 2
                ].detach()
                overlap_query = torch.cat([overlap_query[..., :1], overlap_xy], dim=-1)[
                    0
                ]
            ...
            if annots_train is None:
                ...
                annots["traj_3d"] = annots["traj_3d"][
                    :, i * step_slide : i * step_slide + window_len, sorted_indices, :
                ][..., : len(queries_new), :]
                annots["vis"] = annots["vis"][
                    :, i * step_slide : i * step_slide + window_len, sorted_indices
                ][..., : len(queries_new)]
                annots["poses_gt"] = annots["poses_gt"][
                    :, i * step_slide : i * step_slide + window_len
                ]
                annots["depth_gt"] = annots["depth_gt"][
                    :, i * step_slide : i * step_slide + window_len
                ]
                annots["intrs"] = annots["intrs"][
                    :, i * step_slide : i * step_slide + window_len
                ]
                annots["traj_mat"] = annots["traj_mat"][
                    :, i * step_slide : i * step_slide + window_len
                ]

            if depth is not None:
                annots["depth_gt"] = (
                    depth_unf[i : i + 1].to(segment.device).to(segment.dtype)
                )
            if unc_metric_in is not None:
                annots["unc_metric"] = (
                    unc_metric_unf[i : i + 1].to(segment.device).to(segment.dtype)
                )
            if intrs is not None:
                intr_seg = (
                    intrs_unf[i : i + 1].to(segment.device).to(segment.dtype)[0].clone()
                )
                focal = (
                    intr_seg[:, 0, 0] / segment.shape[-1]
                    + intr_seg[:, 1, 1] / segment.shape[-2]
                ) / 2
                pose_fake = (
                    torch.zeros(1, 8)
                    .to(depth.device)
                    .to(depth.dtype)
                    .repeat(segment.shape[1], 1)
                )
                pose_fake[:, -1] = focal
                pose_fake[:, 3] = 1
            ...
            if extrs is not None:
                ...
                extrs_unf_norm = (
                    torch.inverse(extrs_unf_norm[:1, ...]) @ extrs_unf[i : i + 1][0]
                )
                rot_vec = matrix_to_quaternion(extrs_unf_norm[:, :3, :3])
                annots["poses_gt"] = (
                    torch.zeros(1, rot_vec.shape[0], 7)
                    .to(segment.device)
                    .to(segment.dtype)
                )
                annots["poses_gt"][:, :, 3:7] = rot_vec.to(segment.device).to(
                    segment.dtype
                )[None]
                annots["poses_gt"][:, :, :3] = (
                    extrs_unf_norm[:, :3, 3].to(segment.device).to(segment.dtype)[None]
                )
            ...
            out = self.forward(
                segment,
                pts_q=queries_new,
                pts_q_3d=queries_new_3d,
                overlap_d=overlap_d,
                full_point=full_point,
                fixed_cam=fixed_cam,
                query_no_BA=query_no_BA,
            ...
            left_len = len(
                track3d_pred[
                    i * step_slide : i * step_slide + window_len, :queries_len, :
                ]
            )
            track3d_pred[
                i * step_slide : i * step_slide + window_len, :queries_len, :
            ] = out["rgb_tracks"][0, :left_len, :queries_len, :]
            track2d_pred[
                i * step_slide : i * step_slide + window_len, :queries_len, :
            ] = out["traj_est"][0, :left_len, :queries_len, :3]
            vis_pred[i * step_slide : i * step_slide + window_len, :queries_len, :] = (
                out["vis_est"][0, :left_len, :queries_len, None]
            )
            conf_pred[i * step_slide : i * step_slide + window_len, :queries_len, :] = (
                out["conf_pred"][0, :left_len, :queries_len, None]
            )
            dyn_preds[i * step_slide : i * step_slide + window_len, :queries_len, :] = (
                out["dyn_preds"][0, :left_len, :queries_len, None]
            )
            ...
            c2w_traj[idx_glob : idx_glob + window_len] = prev_c2w @ seg_c2w.to(
                c2w_traj.device
            ).to(c2w_traj.dtype)
        ...
        if self.training:
            ret = {
                ...
                "vis_est": vis_pred[None],
                "conf_pred": conf_pred[None],
    ...
    def forward(
        ...
        x: torch.Tensor,
        annots: Dict = {},
        pts_q: torch.Tensor | None = None,
        pts_q_3d: torch.Tensor | None = None,
        overlap_d: torch.Tensor | None = None,
        full_point=False,
        fixed_cam=False,
        support_frame=0,
        query_no_BA=False,
        ...
        **kwargs,
        ...
        points_map_gt = None
        with torch.no_grad():
            if_gt_depth = ("depth_gt" in annots.keys()) and (
                kwargs.get("stage", 0) == 1 or kwargs.get("stage", 0) == 3
            )
            if not if_gt_depth:
                ...
                for i in range(0, B * T_new, chunk_size):
                    output = self.base_model.infer(
                        x_resize_new.view(B * T_new, -1, H_resize, W_resize)[
                            i : i + chunk_size
                        ]
                    )
                    ...
                    normals_i, normals_mask_i = utils3d.torch.points_to_normals(
                        output["points"], mask=output["mask"]
                    )
                    normals.append(normals_i)
                    normals_mask.append(normals_mask_i)

                metric_depth = (
                    torch.cat(metric_depth, dim=0)
                    .view(B * T_new, 1, H_resize, W_resize)
                    .to(x.dtype)
                )
                ...
                mask_roi = (metric_depth > _depth_roi[0]) & (
                    metric_depth < _depth_roi[1]
                )
                mask = mask * mask_roi
                mask = (
                    mask
                    * (
                        ~(
                            utils3d.torch.depth_edge(
                                metric_depth, rtol=0.03, mask=mask.bool()
                            )
                        )
                    )
                    * normals_mask[:, None, ...]
                )
                points_map = depth_to_points_colmap(
                    metric_depth.squeeze(1), intrs.view(B * T_new, 3, 3)
                )
                unc_metric = (
                    torch.cat(unc_metric, dim=0)
                    .view(B * T_new, 1, H_resize, W_resize)
                    .to(x.dtype)
                )
                unc_metric *= mask
                    ...
                    unc_metric = (
                        ~(
                            utils3d.torch.depth_edge(
                                metric_depth,
                                rtol=0.1,
                                mask=torch.ones_like(metric_depth).bool(),
                            )
                        )
                    ).float() * (metric_depth != 0)
                if cache is not None:
                    ...
                    points_map = torch.cat(
                        [cache["points_map"].permute(0, 2, 3, 1), points_map], dim=0
                    )
                    metric_depth = torch.cat(
                        [cache["metric_depth"], metric_depth], dim=0
                    )

            if "poses_gt" in annots.keys():
                intrs, c2w_traj_gt = pose_enc2mat(
                    annots["poses_gt"], H_resize, W_resize, self.resolution
                )
            else:
                c2w_traj_gt = None
            ...
            if "depth_gt" in annots.keys():
                ...
                metric_depth_gt = F.interpolate(
                    metric_depth_gt, size=(H_resize, W_resize), mode="nearest"
                )
                ...
                mask_roi = (metric_depth_gt > _depth_roi[0]) & (
                    metric_depth_gt < _depth_roi[1]
                )
                # if (upper_bound > 200).any():
                #     import pdb; pdb.set_trace()
                if kwargs.get("stage", 0) == 2:
                    unc_metric = (
                        (metric_depth_gt > 0) * (mask_roi) * (unc_metric > 0.5)
                    ).float()
                    ...
                    unc_metric *= (
                        ~(
                            utils3d.torch.depth_edge(
                                metric_depth_gt, rtol=0.03, mask=mask_roi.bool()
                            )
                        )
                    ).float()
                    # filter the sky
                ...
                if "unc_metric" in annots.keys():
                    unc_metric_ = F.interpolate(
                        annots["unc_metric"].permute(1, 0, 2, 3),
                ...
                if if_gt_depth:
                    points_map = depth_to_points_colmap(
                        metric_depth_gt.squeeze(1), intrs.view(B * T, 3, 3)
                    )
                    metric_depth = metric_depth_gt
                    points_map_gt = points_map
                else:
                    points_map_gt = depth_to_points_colmap(
                        metric_depth_gt.squeeze(1), intrs.view(B * T, 3, 3)
                    )
        ...
        if self.training:
            if (annots["vis"].sum() > 0) and (
                kwargs.get("stage", 0) == 1 or kwargs.get("stage", 0) == 3
            ):
                traj3d = annots["traj_3d"]
                if (kwargs.get("stage", 0) == 1) and (
                    not annots.get("custom_vid", False)
                ):
                    support_pts_q = get_track_points(
                        H_resize,
                        W_resize,
                        T,
                        x.device,
                        query_size=self.track_num // 2,
                    ...
                    support_pts_q = get_track_points(
                        ...
                        T,
                        x.device,
                        query_size=random.randint(32, 256),
                        support_frame=self.support_frame,
                        unc_metric=unc_metric,
                        mode="incremental",
                    )[None]
        ...
        if regular_track:
            if pts_q is None:
                pts_q = get_track_points(
                    H_resize,
                    W_resize,
                    T,
                    x.device,
                    query_size=self.track_num,
                    support_frame=self.support_frame,
                    unc_metric=unc_metric,
                    mode="incremental" if self.training else "incremental",
                )[None]
                support_pts_q = None
            else:
                pts_q = pts_q[None, None]
                ...
                if pts_q_3d is not None:
                    ...
                    if (not full_point) and (overlap_d is None):
                        ...
                        if (pts_q_unc < 0.5).sum() > 0:
                            ...
                            indices = torch.randint(
                                0, pts_q.shape[2], (pad_num,), device=pts_q.device
                            )
                            pad_pts = indices
            ...
            points_map = depth_to_points_colmap(
                metric_depth.squeeze(1), intrs.view(B * T, 3, 3)
            )
            (
                ret_track,
                dyn_preds,
                ...
                cache=cache,
                overlap_d=overlap_d,
                cam_gt=c2w_traj_gt if kwargs.get("stage", 0) == 1 else None,
                prec_fx=prec_fx,
                prec_fy=prec_fy,
                support_pts_q=support_pts_q,
                custom_vid=custom_vid,
        ...
        # output
        depth = F.interpolate(
            metric_depth, size=(H, W), mode="bilinear", align_corners=True
        ).squeeze(1)
        points_map = F.interpolate(
            points_map, size=(H, W), mode="bilinear", align_corners=True
        ).squeeze(1)
        unc_metric = F.interpolate(
            unc_metric, size=(H, W), mode="bilinear", align_corners=True
        ).squeeze(1)
        ...
        if self.training:
            ...
            ret = {
                ...
                "poses_pred": c2w_traj,
                "dyn_preds": dyn_preds,
                "traj_est": final_tracks,
                "conf_loss": conf_loss,
                "imgs_raw": imgs_raw,
                "rgb_tracks": rgb_tracks,
